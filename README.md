# 🧘‍♂️ Work Mode — Расширение для VS Code

Минималистичный режим работы для **Visual Studio Code** с **секундомером**, **автоматическими бэкапами** (включая **несохранённые правки**) и **журналом сессий**.
...
- **Инкрементные бэкапы (каждую минуту)**  
  - Сохраняются **все изменённые файлы**, включая несохранённые (dirty).  
...
- **Локальные метки времени (без UTC-сдвига)**  
  Все даты и имена архивов соответствуют вашему часовому поясу.
